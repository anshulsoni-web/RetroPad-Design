...
Then assigned values to key variables such as length, heigh, and thickness that will be used to define the body further. 
First, building the base sketch using the "buildsketch" command for 'Top Shell', part of the design, then chamfering the edges by simply selecting the base sketch. Then, extrude it by defining it as a base body using the "buildpart" command. After that, using the base sketch as a base plane, moving ahead with derawing sketches and extruding them to shape the body as desired. 
...
To apply chamfer or fillet, the plane is defined first, and then, simply the "chamfer" command is applied. 
To make a cylinder, circle, rectangle, ellipse, or any shape, its "centre point" is fed "location" command  and then defining the extrude or extrude cut using extrude mode.ADD or mode.subtract command.
For a rectangle, the centre point rectangle is by default considered in this library, later in parentheses, the first "length of rectangle along X axis" is fed and the latter is considered as "width along Y axis". This is the same for an ellipse, the first radius is processed along X axis and then next along Y axis separated by a comma. 
